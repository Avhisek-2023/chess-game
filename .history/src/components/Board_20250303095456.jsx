  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-[90vw] sm:max-w-[80vw] md:max-w-[60vw] lg:max-w-[40vw] min-w-[300px] aspect-square mx-auto my-auto">
        <div className="absolute -top-4 left-0 w-full grid grid-cols-8 text-center text-white  font-sans md:text-xl ">
          {columns.map((col, index) => (
            <div key={index} className="py-1 rotate-180">
              {col}
            </div>
          ))}
        </div>
        ...
        >
          {board.map((row, i) => (
            <>
              ...
              <div
                key={i}
                className="absolute -right-6 md:-left-6  font-sans md:text-xl text-white"
                style={{
                  top: `${(i + 0.5) * 12.5}%`,
                  transform: "translateY(-50%)",
                  rotate: "180deg",
