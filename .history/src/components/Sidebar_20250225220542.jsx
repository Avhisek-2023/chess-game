...
const Sidebar = () => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full overflow-y-auto w-64 bg-sidebar text-white shadow-lg transition-all ease-in-out duration-800 z-[100] ${
    ...
    >
      ...
      <ul className="p-4">
        {moveHistory.map((move, index) => (
          <li
            key={index}
            className={` py-1 font-bold textt-xl ${
              move.from === "player" ? "text-white" : "text-slate-900 "
            }`}
          >
            {index + 1}. {move.move}
            <span> [{move.from}]</span>
