...
const Sidebar = () => {
  ...
  return (
    <div
      className={`fixed top-0 right-0 h-full overflow-y-auto w-64 bg-slate-600 text-white shadow-lg transition-all ease-in-out duration-800 z-[100] ${
      ...
      <ul className="p-4">
        {moveHistory.map((move, index) => (
          <li
            key={index}
            className={` py-1 ${
              move.from === "player" ? "text-white" : "text-slate-900 font-bold"
            }`}
          >
            {index + 1}. {move.move}
            <span> [{move.from}]</span>
